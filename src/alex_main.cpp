...
GraphInfo get_graph_info(int argc, vector<string> argv) {
...
int launch_test(const spMtx<int> &gr, const GraphInfo &info, int argc, vector<string> argv) {
    ...
    if (benchmarkAlgorithm == "bc") {
        if (argc < 7 || (batch_size = stoll(argv[6])) == 0) {
...
string get_graph_val_type(string filename, const GraphInfo &info) {
...
// argv[2] - путь к файлу, куда запишется лог (реально в лог ничего не пишется, возможно)
...
int read_graph_and_launch_test(const GraphInfo &info, int argc, vector<string> argv) {
    string action = argv[3];
    spMtx<ValType> gr(argv[1].data(), info.format.c_str());
...
int main() {
    vector<string> argv = {"", "../Graphs/G43.mtx", "../output.txt", "launch", "bc", "seq", "1"};
    int argc = argv.size();
